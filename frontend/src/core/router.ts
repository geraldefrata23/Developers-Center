...
function pathFor(mode: AppMode, id: string | null): string {
  if (mode === "gateway") return "/gateway/" + (id || "");
  if (mode === "snap") return "/snap/" + (id || "");
  return "/";
}

/** Pushes (or replaces) a history entry for (mode, id). No-ops if we're already there. */
function navigate(mode: AppMode, id: string | null, opts?: { replace?: boolean }): void {
...
function parse(): RouteState {
  ...
  if (parts[0] === "gateway") return { mode: "gateway", id: parts[1] || null };
  if (parts[0] === "snap") return { mode: "snap", id: parts[1] || null };
