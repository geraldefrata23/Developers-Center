...
function renderAuthGuide(): string {
  ...
  ${specTable(
    ["Credentials from ShopeePay", "Description"],
    [
  ...
  ${specTable(
    ["Partner's Credentials", "Description"],
    [
  ...
  ${signCard("🔐","Access Token","Asymmetric · SHA256withRSA",
    "Signed with <b>Partner's Private Key</b> Used once, to exchange your Client Key for a Bearer token.",
    "stringToSign = clientKey + \"|\" + timestamp\nsignature   = SHA256withRSA(privateKey, stringToSign)")}

  ${signCard("🔑","Transactional (SNAP)","Symmetric · HMAC-SHA512",
    "Signed with <b>Client Secret</b> Used on every QR, Checkout, Link &amp; Pay, Link &amp; Pay (API Based), Disbursement and Account Linking call once you hold an access token.",
  ...
  ${signCard("🔗","Get Auth Code (SNAP)","Symmetric · HMAC-SHA512, fixed body hash",
    "Same formula as above, but since this is a GET request with empty body, the body hash is always sha256(\"{}\") — and the query string counts as part of the signed path.",
  ...
  ${signCard("📥","Callback Signature Validation","Asymmetric · SHA256withRSA, verify only",
    "The one signature flow that runs in <b>Reverse</b>ShopeePay signs every inbound callback (Notify Transaction Status) with <b>Private Key</b>Partners verify it with <b>ShopeePay's Public Key</b> from onboarding. Never act on a callback whose signature doesn't verify.",
