...
function renderAuthGuideGateway(): string {
  ...
  ${signCard("🌐","Every Gateway Service call","Symmetric · HMAC-SHA256, raw body",
    "Signed with <b>your AirPay Secret Key</b>, over the exact bytes of the JSON request body. For GET requests (Get Checkout ID Status, Get Refund Status) the body is empty, so the signature is computed over an empty string.",
  ...
  ${specTable(
    ["Type", "Empty value"],
    [["Integer","0"],["String",`Empty string, or "0" if the field represents a number`],["Object","null"],["Array","Empty array"],["Boolean","false"]]
  )}
