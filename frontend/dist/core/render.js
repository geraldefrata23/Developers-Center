...
export const App = {
    ...
    mode: "landing",
    nav() { return App.mode === "gateway" ? Gateway.NAV : Snap.NAV; },
    ep() { return App.mode === "gateway" ? Gateway.EP : Snap.EP; },
    staticPages() { return App.mode === "gateway" ? Gateway.STATIC : Snap.STATIC; },
    groupOf(id) {
    ...
    toggleGroup(name) {
        if (App.expandedGroups.has(name))
            App.expandedGroups.delete(name);
        else
            App.expandedGroups.add(name);
        Sidebar.render();
    },
...
export const Sidebar = {
    render() {
        ...
        <div class="side-items ${isOpen ? "open" : ""}">`;
            g.items.forEach((it) => {
                const active = it.id === App.currentId ? "active" : "";
                html += `<div class="side-link ${active}" onclick="App.go('${it.id}')">${it.method ? `<span class="m ${it.method}">${it.method}</span>` : ""}<span class="label">${DOM.esc(it.label)}</span></div>`;
            });
            html += `</div></div>`;
        });
...
export const Docs = {
    render(id) {
        ...
        const calloutIcon = { blue: "ℹ️" };
        const calloutHtml = ep.callout
            ? `<div class="callout ${ep.callout.type === "blue" ? "blue" : ""}"><div>${calloutIcon[ep.callout.type] || "ℹ️"}</div><div><b>${DOM.esc(ep.callout.title)}</b>${DOM.esc(ep.callout.body)}</div></div>`
            : "";
        let rcHtml = "";
        ep.rc.forEach((group) => {
