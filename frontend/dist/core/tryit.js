...
export const TryIt = (function () {
    ...
    function buildDefaultBody(ep) {
        if (ep.sampleReq === null || ep.sampleReq === undefined)
            return ep.sampleReq;
            ...
            if (node && typeof node === "object") {
                Object.keys(node).forEach((k) => {
                    ...
                    if (k === "accountToken") {
                        ...
                        node[k] = creds.lastAccountToken || v;
                    }
                    else if (IDENTITY_KEYS[k]) {
                        const src = IDENTITY_KEYS[k] === "merchantId" ? creds.merchantId : creds.storeId;
                        node[k] = src || v;
                    }
    ...
    function section(key, titleText, bodyHtml, opts) {
    ...
    function getCopyText(key) {
        ...
        if (key === "resp") {
            const el = document.getElementById("respWrap");
            return el ? el.dataset.raw || "" : "";
        }
        if (key === "sts") {
        ...
        if (key === "token") {
            const el = document.getElementById("tryToken");
            return el ? el.value : "";
        }
        if (key === "body") {
            const ta = document.getElementById("tryBody");
            if (ta)
                return ta.value;
            const pp = document.getElementById("tryPathParam");
            if (pp)
                return pp.value;
            const qps = Array.from(document.querySelectorAll(".qp-input"));
            if (qps.length)
                return qps.map((i) => `${i.dataset.qp}=${i.value}`).join("&");
            return "";
        }
        return "";
    }
    ...
    function flashCopied(key) {
        ...
        if (!scope)
            return;
        const btn = scope.querySelector(".copy-btn");
        if (!btn)
            return;
    ...
    function render(id) {
        const ep = App.ep()[id];
        const wrap = document.getElementById("tryit");
        if (!ep) {
        ...
        <input class="inp" id="tryPathParam" placeholder="${DOM.esc(ep.pathParam.sample)}" value="">`;
            ...
            <div class="field-hint" style="margin-bottom:4px;text-transform:none;">${DOM.esc(q.name)}${q.req === "M" ? ' <span style="color:var(--accent-ink);font-weight:800;">*</span>' : ""}</div>
        ...
        const accountTokenNote = ACCOUNT_TOKEN_REQUIRED.has(id)
            ? `<div class="env-note" style="margin-top:10px;"><b>${I18N.t("tryit.accountToken")}:</b> ${Credentials.hasAccountToken()
                ? `<span style="color:var(--get);font-weight:700;">${I18N.t("tryit.credsSaved")}</span>`
                : `<span style="color:var(--accent-ink);font-weight:700;">${I18N.t("tryit.credsMissing")}</span> — <a onclick="App.go('link-authcode')">${I18N.t("guard.cta")}</a>`}</div>`
            : "";
        const headersSection = section("hdr", I18N.t("tryit.headers"), `<div class="hdr-block clickcopy" id="hdrPreview" onclick="TryIt.copy('hdr')"><div>${I18N.t("tryit.headersPlaceholder")}</div></div>`);
        const bodySection = section("body", inputTitle, `${inputSectionHtml}${accountTokenNote}`);
        const respSection = section("resp", "Response", `<div id="respWrap" class="clickcopy" onclick="TryIt.copy('resp')"></div>`);
        const stsSection = section("sts", I18N.t("tryit.sts"), `<div class="string-to-sign clickcopy" id="stsBox" onclick="TryIt.copy('sts')">—</div>`);
        const tokenSection = hasToken
            ? section("token", I18N.t("tryit.accessToken"), `<input class="inp" id="tryToken" value="${DOM.esc(creds.lastAccessToken || "run Get Access Token first")}">`)
            : "";
        wrap.innerHTML = `
      ...
      <div class="env-note">Credentials: <b>${(ep.sign === "airpay" ? creds.airpaySecret : creds.clientSecret || creds.privateKey) ? I18N.t("tryit.credsSaved") : I18N.t("tryit.credsMissing")}</b> · <a onclick="Credentials.openModal()">${I18N.t("tryit.editCreds")}</a></div>
    `;
        wireBodyEditor();
    }
    ...
    function showAccountTokenGuard() {
    ...
    async function send(id) {
        ...
        const payload = { endpointId: id, credentials: creds2payload(Credentials.get()) };
        if (ep.noBody) {
            payload.pathParam = document.getElementById("tryPathParam").value.trim();
        }
        else if (ep.sign === "hmac-get" && ep.queryParams) {
            payload.queryParams = {};
            ep.queryParams.forEach((q) => {
                const v = document.getElementById("qp_" + q.name).value.trim();
                if (v)
                    payload.queryParams[q.name] = v;
            });
        }
        else {
            try {
                payload.body = JSON.parse(document.getElementById("tryBody").value);
            }
            catch (e) {
        ...
        if (ep.sign === "hmac" || ep.sign === "hmac-get") {
            payload.accessToken = document.getElementById("tryToken").value.trim();
        }
        let result;
        try {
    ...
    function creds2payload(creds) {
        const c = { ...creds };
        delete c.lastAccessToken;
        delete c.lastAccountToken;
        return c;
    }
    function renderHeadersAndSig(result) {
        const hdrEl = document.getElementById("hdrPreview");
        const stsEl = document.getElementById("stsBox");
        const urlEl = document.getElementById("fullUrlText");
        if (urlEl && result.requestUrl)
            urlEl.textContent = result.requestUrl;
        if (stsEl && result.stringToSign)
            stsEl.textContent = result.stringToSign;
