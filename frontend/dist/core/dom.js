...
function jsonHighlightRaw(text) {
    let s = esc(text);
...
function firstErrorRow(rc) {
    for (const group of rc) {
        for (const row of group.rows) {
            if (row[3] === "err" && row[0] !== "—")
                return { http: row[0], code: row[1], message: row[2] };
        }
    }
    return null;
}
